...
F = ((kappa + (1 - ind) * kappa_lin(T)) * dot(grad(T), grad(v)) - ind * f(T) * v)*x[1]*dx
F  = action(F, T_)
...
res_file = File('heat2D_static/solution.pvd')
...
temps = [T_(Point(x, r/100)) for x in X]
plt.figure()
...
#График распределения температур в центральном сечении цилиндра
Y = np.linspace(r/2, R/2, 1000)
temps = [T_(Point(L/2, y)) for y in Y]
plt.figure()
plt.plot(Y * 10**3, temps)
plt.title("График распределения температур в центральном сечении цилиндра")
plt.xlabel("r, мм")
plt.ylabel("T(r), \u00b0 C")
plt.grid(visible=True)
plt.show()
