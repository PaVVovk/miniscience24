...
protected:
    ...
    // глобальное время
    double T = 0;
    //отвечает за амплитуду и скорость движений кота в пл-ти OXZ
    double omega = 400;
    //отвечает за частоту колебаний кота в плоскости OXZ
    double osc_ratio = 0.05;
    //отвечает за частоту растяжений кота вдоль OY
    double pull_ratio = osc_ratio / 2;

    //почти непрерывная функция, которая растягивает только заднюю часть кота
    double pull_func(double y) {
        if (y > 9.5){ //увы, это magic numbers... подобранные специально для моей STL-модели
            return 1500 * log(y);
        }
        return 750 * (std::tanh(y - 7.6) + 1);
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = sin(pointY * 2 * M_PI);
            //Распределение скоростей
            double vx = omega * pointZ;
            double vy = pull_func(pointY);
            double vz = - omega * pointX;
